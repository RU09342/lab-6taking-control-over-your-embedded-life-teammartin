...
int main(void)
{
    ...
    for (;;)
    {
        ...
        if (ADC10MEM < 50) {			// if less than half of the ADC10MEM range of values
			...
			TA0CCR0 = ADC10MEM * 2; // make the ADC10 value equal to both timer register that create the frequency until after 100. Keep duty cycle at 50%.
            TA0CCR1 = ADC10MEM; 
        }
		if ((ADC10MEM >= 50) & (ADC10MEM < 150)) {
            P1OUT &= ~BIT0; P1OUT |= BIT6; //red LED off. green LED on.
			TA0CCR0 = ADC10MEM * 2;
            TA0CCR1 = ADC10MEM;
        }
        if (ADC10MEM >= 150) {					//ADC10MEM can go slightly higher than 100, but the cap is made here.
